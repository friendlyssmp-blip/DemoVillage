...
import { DailyMission, LiveEvent, SeasonPassReward } from '../../core/types';
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="w-full h-[90vh] bg-slate-950 border-t border-white/10 rounded-t-[50px] flex flex-col overflow-hidden shadow-2xl relative"
      ...
      >
        ...
        <div className="p-8 pb-4 shrink-0">
          <div className="flex justify-between items-center mb-6">
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter font-display">{season?.name || 'DOMINION'}</h2>
              </div>
              <p className="text-[8px] font-black text-white/30 uppercase tracking-[0.3em] font-mono mt-1">SEASONAL PROTOCOL ACTIVE</p>
            </div>
            <button 
              onClick={onClose}
              className="p-3 bg-white/5 border border-white/10 rounded-2xl text-white/40 active:scale-90 transition-all"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex bg-white/5 p-1 rounded-[24px] border border-white/5">
            {[
              { id: 'pass', label: 'BATTLE PASS', icon: Crown },
              { id: 'missions', label: 'DAILY TASKS', icon: Target },
              { id: 'events', label: 'LIVE EVENTS', icon: Flame },
            ].map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as any)}
                className={`flex-1 flex flex-col items-center gap-1.5 py-3 rounded-[20px] transition-all ${
                  activeTab === tab.id 
              ...
              >
                <tab.icon size={12} />
                <span className="text-[8px] font-black uppercase tracking-widest font-display italic leading-none">{tab.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 no-scrollbar pb-10">
          <AnimatePresence mode="wait">
            {activeTab === 'pass' && (
              ...
              >
                {/* Level Display */}
                <div className="p-5 bg-zinc-800/50 border border-white/5 rounded-[32px] flex flex-col gap-4">
                  <div className="flex items-center gap-4">
                    <div className="shrink-0 flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-600 shadow-lg relative">
                      <Crown size={24} className="text-white" />
                      <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-black rounded-full border-2 border-zinc-800 flex items-center justify-center">
                         <span className="text-white font-black text-[10px]">{seasonPass.level}</span>
                      </div>
                    </div>
                    <div className="flex-1 space-y-1.5">
                      <div className="flex justify-between items-end">
                        <span className="text-sm font-black text-white uppercase italic">RANK {seasonPass.level}</span>
                        <span className="text-[8px] font-black text-white/40 uppercase tracking-widest">{xpRemaining} XP LEFT</span>
                      </div>
                  ...
                  {!seasonPass.isPremium && (
                    <button className="w-full py-3 bg-gradient-to-r from-amber-400 to-orange-500 text-black font-black rounded-xl uppercase italic tracking-widest text-[9px] active:scale-95 transition-all">
                      UNLOCK PREMIUM PASS
                    </button>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                   {PASS_REWARDS.map((reward, i) => {
                     ...
                     return (
                       <div 
                         key={i}
                         className={`p-6 rounded-[32px] border transition-all flex items-center gap-5 relative overflow-hidden backdrop-blur-md ${
                           isClaimed 
                       ...
                       >
                         {reward.isPremium && !isClaimed && (
                           <div className="absolute top-0 left-0 w-1 h-full bg-amber-500 shadow-[2px_0_10px_rgba(245,158,11,0.3)]" />
                         )}

                         <div className={`w-14 h-14 shrink-0 rounded-2xl flex items-center justify-center shadow-2xl ${reward.isPremium ? 'bg-amber-400 text-slate-950' : 'bg-white/5 text-white'}`}>
                            {reward.type === 'resource' && <Gem size={20} />}
                            {reward.type === 'gold' && <Zap size={20} />}
                            {reward.type === 'skin' && <Crown size={20} />}
                            {reward.type === 'boost' && <Flame size={20} />}
                         </div>
                         
                         <div className="flex-1 min-w-0">
                           <h4 className="text-sm font-black text-white uppercase italic truncate font-display tracking-tight leading-tight">{reward.name}</h4>
                           <span className={`text-[8px] font-black uppercase tracking-[0.2em] font-mono ${reward.isPremium ? 'text-amber-500' : 'text-white/20'}`}>
                             {reward.isPremium ? 'PREMIUM ACCESS' : 'PUBLIC DATA'} • TIER {reward.level}
                           </span>
                         </div>

                         {isClaimed ? (
                            <CheckCircle2 size={24} className="text-brand-500 shrink-0 opacity-50" />
                         ) : (
                           <button 
                             disabled={!canClaim}
                             onClick={() => handleClaimReward(reward.level, reward.isPremium)}
                             className={`px-6 py-3 rounded-xl font-black uppercase tracking-widest text-[9px] transition-all shrink-0 font-display italic ${
                               canClaim 
                                ? 'bg-white text-slate-950 shadow-xl active:scale-95' 
                                : 'bg-white/5 text-white/10'
                             }`}
                           >
                             {isUnlocked ? 'EXTRACT' : 'LOCKED'}
                           </button>
                         )}
                       </div>
                     );
