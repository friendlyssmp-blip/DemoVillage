...
import { InstancedWorkers } from './economy/InstancedWorkers';
import { ParticleSystem } from './combat/ParticleSystem';
import * as THREE from 'three';

function RainEffect() {
  const count = 400; // More particles, but way faster performance
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
        // Procedural falling rain with wrapping
        const seed = i * 133.7;
        const x = ((Math.sin(seed) * 50) + 50) % 80 - 40;
        const z = ((Math.cos(seed * 0.7) * 50) + 50) % 80 - 40;
        const speed = 15 + (Math.sin(seed * 0.1) * 5);
        const y = 30 - ((time * speed + seed) % 30);
...
function SnowEffect() {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
        const seed = i * 42.1;
        const x = ((Math.sin(seed) * 50) + 50) % 80 - 40 + Math.sin(time * 0.5 + seed) * 2;
        const z = ((Math.cos(seed * 0.8) * 50) + 50) % 80 - 40 + Math.cos(time * 0.3 + seed) * 2;
        const speed = 2 + (Math.sin(seed * 0.2) * 1);
        const y = 30 - ((time * speed + seed) % 30);
      ...
      {weather === 'stormy' && (
        <>
          ...
          <Float speed={20} floatIntensity={0} rotationIntensity={0}>
             <pointLight intensity={Math.random() > 0.985 ? 1000 : 0} position={[Math.random() * 40 - 20, 30, Math.random() * 40 - 20]} color="#ffffff" />
          </Float>
        </>
      )}
    </group>
