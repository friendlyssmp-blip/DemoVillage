...
import { ResourceType, ShopItem } from '../../types';

import { audioService } from '../../services/audioService';

export function Shop() {
  const { setViewMode, resources, dailyShop, buyShopItem } = useGameStore();
  const [timeLeft, setTimeLeft] = useState('');

  useEffect(() => {
    const timer = setInterval(() => {
      const now = Date.now();
      const nextReset = dailyShop.lastReset + (24 * 60 * 60 * 1000);
